...
Korean Law MCP Server v3.4.0 - 법제처 41개 API → 15개 통합 도구 (내부 91개) + 7개 시나리오 확장 + 자연어 CLI + HTTP stateless 모드 + 판례 응답 토큰 74% 감축
...
├── tool-registry.ts      # 91개 도구 등록, V3_EXPOSED 15개만 노출
...
상세 정보는 별도 문서 참조:
- [docs/API.md](docs/API.md) - 도구 레퍼런스 (15개 노출 + 76개 execute_tool 접근)
